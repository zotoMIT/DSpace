...
(function($) {
    ...
    $.fn.mapael.setTooltip = function(elem, $tooltip) {
        ...
        function offsetCss(e) {
            var adjustOffset = $('.horizontal-slider').offset();
            // this ancestor contains a 'transform' property
            // it hijacks the point of reference for the tooltip
            // because the tooltip is position:fixed
            // (https://developer.mozilla.org/en-US/docs/Web/CSS/position)

            return {
                "left": Math.min(
                    containerY2 - $tooltip.outerWidth() - 5,
                    e.pageX + 10 - $(window).scrollLeft()
                ) - adjustOffset.left,
                "top": e.pageY + 20 //- $(window).scrollTop()
                - adjustOffset.top
            };
        }
